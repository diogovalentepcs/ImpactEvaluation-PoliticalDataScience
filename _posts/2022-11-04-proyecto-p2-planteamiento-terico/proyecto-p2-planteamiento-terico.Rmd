...
$$ y_t = α_0 + α_1 \cdot y_t^b + α_2 \cdot x_t + β \cdot T_t + \sum δ_t \cdot M_t + ε_t $$
donde $y_t$ es la concentración de CO en el período (hora) t en logs. Las variables explicativas son las siguientes:

* $y_t^b$ es la contaminación de fondo antes de que se forme el pico, que corresponde a la contaminación nocturna (consideramos el promedio de 4 registros consecutivos de CO que, en promedio, mostraron la menor dispersión a lo largo de la muestra: 1–5 am;
